...
AdamOptimizer::AdamOptimizer(double start_step, double beta1, double beta2, double epsilon) : start_step_(start_step), beta1_(beta1), beta2_(beta2), epsilon_(epsilon), beta1_t_(beta1_), beta2_t_(beta2_) {}
...
void AdamOptimizer::trainOneBatch(std::list<Layer>& layers, const LossFunction& l, const std::vector<Element> &dataset, ConstElemIterator start,
    ...
    for (std::list<Layer>::reverse_iterator layer = layers.rbegin(); layer != layers.rend(); ++layer, ++it_x, ++it_m, ++it_v) {
        ...
        *it_v = {beta2_ * it_v->a + (1 - beta2_) * grad_a.cwiseProduct(grad_a),
        ...
        layer->updA(current_step * it_m->a.cwiseProduct((it_v->a.cwiseSqrt() + Matrix::Ones(it_v->a.rows(), it_v->a.cols()) * epsilon_).unaryExpr([](double x) { return 1 / x; })));
        layer->updB(current_step * it_m->b.cwiseProduct((it_v->b.cwiseSqrt() + Matrix::Ones(it_v->b.rows(), it_v->b.cols()) * epsilon_).unaryExpr([](double x) { return 1 / x; })));
    }
}
}
